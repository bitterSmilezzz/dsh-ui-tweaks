...
function mapFiles(files, limits) {
  ...
  const rows = [];
  let totalBytes = 0;
  // Raw paths are collected for the post-loop ancestor sweep below; the
  // per-file conflict check is inline in the loop (see both comments there).
  const rawPathList = [];
  for (const [index, input] of files.entries()) {
    ...
    rawPathList.push(rawPath);
    // File/directory prefix conflicts, cheap half: if an existing path is an
    // ancestor of this one (this path starts with `${existing}/`), report it
    // right here — same error, same ordering as before. O(maxDepth) set
    // lookups per file instead of scanning every existing path (the previous
    // O(n²) nested loop was ≈50M comparisons at the 10k-file limit).
    let slash = rawPath.lastIndexOf('/');
    while (slash !== -1) {
      const ancestor = rawPath.slice(0, slash);
      if (rawPaths.has(ancestor)) {
        throw new Error(`file/directory attachment path conflict: ${ancestor} and ${rawPath}`);
      }
  ...
  // …and the cheap other half: a path that was added later can be an ancestor
  // of an earlier one (e.g. "a/b" after "a/b/c"). The per-file check above
  // can't see that, so sweep once more: for every path, look up each proper
  // ancestor prefix in the set. O(n·maxDepth) with O(1) set lookups — no sort
  // (a sorted adjacent-pair scan would NOT be sound: a segment containing a
  // char < "/" — space, "!", "+", … — can sort between a path and its
  // descendant and hide the pair).
  for (const rawPath of rawPathList) {
    let slash = rawPath.lastIndexOf('/');
    while (slash !== -1) {
      const ancestor = rawPath.slice(0, slash);
      if (rawPaths.has(ancestor)) {
        // Report the same pair in the same order the original loop did
        // (existing descendant first, newly added ancestor second).
        throw new Error(`file/directory attachment path conflict: ${rawPath} and ${ancestor}`);
      }
      slash = ancestor.lastIndexOf('/');
    }
  }
...
export function apply(ctx, config = {}) {
  ...
  const abortBatch = async (batch) => {
    ...
    try {
      ...
      ctx.logger.warn(`dsh-paste-input: staging cleanup failed: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
  };
    ...
    try {
      ...
      if (req.method === 'POST' && commitMatch !== null) {
        ...
        try {
          ...
          // A failed commit must not leave the batch entry or its staging
          // directory behind (they would otherwise wait for the 1h TTL sweep).
          await rm(batch.stagingRoot, { force: true, recursive: true }).catch(() => {});
          throw cause;
        } finally {
          batches.delete(batch.id);
        }
